...
const stats = [
  { label: 'Total Credits Raised', value: '45,280+', icon: Coins, color: 'text-amber-500' },
  { label: 'Approved Projects', value: '184+', icon: FolderCheck, color: 'text-indigo-600' },
  { label: 'Global Backers', value: '2,400+', icon: Users, color: 'text-sky-600' },
  { label: 'Payout Success Rate', value: '99.8%', icon: ShieldCheck, color: 'text-emerald-600' }
];
              ...
              return (
                <div key={idx} className={`pt-6 sm:pt-0 ${idx !== 0 ? 'sm:pl-8' : ''} flex flex-col items-center sm:items-start text-center sm:text-left`}>
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <span className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">{item.value}</span>
                  </div>
                  <p className="text-xs font-semibold text-slate-500 tracking-wide uppercase">{item.label}</p>
                </div>
              );
            })}
          </div>

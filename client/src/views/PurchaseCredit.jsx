...
const supporterPlans = [
  {
    ...
    period: '/100 credits',
    credits: 100,
    description: 'Essential features for getting started and backing your initial campaign search.',
    icon: <User className="w-4 h-4 text-slate-500 dark:text-slate-300" />,
    features: [
  ...
  },
  {
    ...
    icon: <Star className="w-4 h-4 text-blue-500 dark:text-blue-400" />,
    features: [
      '300 Platform Credits',
      'Save 16% per platform credit',
      'Advanced campaign tracking dashboard',
  ...
  },
  {
    ...
    cta: 'Go Premium',
    popular: false,
    btnClass: 'bg-slate-900 dark:bg-[#222834] hover:bg-slate-800 dark:hover:bg-[#2c3444] text-white dark:text-slate-200 border border-slate-700 dark:border-[#2e3748]'
  }
];
...
const creatorPlans = [
  {
    ...
    period: '/200 credits',
    credits: 200,
    description: 'Ideal baseline solution matching startups launching their initial project pipeline.',
    icon: <Briefcase className="w-4 h-4 text-slate-500 dark:text-slate-300" />,
    features: [
      '200 Creator Credits',
      'Basic backer management pipeline',
  ...
  },
  {
    ...
    icon: <Rocket className="w-4 h-4 text-blue-500 dark:text-blue-400" />,
    features: [
      '600 Creator Credits',
  ...
  },
  {
    ...
    icon: <Crown className="w-4 h-4 text-amber-500 dark:text-amber-400" />,
    features: [
...
const faqs = [
  {
    question: 'Can I use my credits at any time?',
...
export default function PurchaseCreditView() {
  ...
  const renderCard = (plan, idx) => (
    <div
      key={idx}
      className={`relative bg-white dark:bg-[#131722] border rounded-3xl p-6 sm:p-7 flex flex-col justify-between transition-all duration-300 ${
        plan.popular
          ? 'border-2 border-[#2563eb] shadow-2xl shadow-blue-600/15 lg:-translate-y-1'
          : 'border-slate-200 dark:border-[#1e2638] shadow-sm hover:border-indigo-500/40 dark:hover:border-[#2d384e]'
      }`}
    >
      {plan.popular && (
      ...
      <div>
        <div className="flex items-center justify-between gap-2 mb-3">
          ...
          <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-[#1c2230] border border-slate-200 dark:border-[#283248] flex items-center justify-center text-slate-600 dark:text-slate-300 shrink-0">
            {plan.icon}
          </div>
        </div>
        ...
        <ul className="space-y-3 pt-2">
          {plan.features.map((feature, fIdx) => (
            <li key={fIdx} className="flex items-start gap-2.5 text-xs text-slate-700 dark:text-slate-200">
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50 dark:bg-[#07090e] text-slate-900 dark:text-white py-16 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
      <div className="max-w-6xl mx-auto space-y-16">
        ...
        {successMsg && (
          <div className="max-w-3xl mx-auto p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-700 text-emerald-900 dark:text-emerald-200 text-xs sm:text-sm font-semibold flex items-center justify-between shadow-md">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 border-b border-slate-200 dark:border-[#1e2638] pb-3">
            ...
            <h2 className="text-base font-bold text-slate-900 dark:text-white uppercase tracking-wider text-xs">
              For Supporters (Backer Packages)
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
            {supporterPlans.map((plan, idx) => renderCard(plan, idx))}
          </div>
        </div>
        ...
        <div className="space-y-6 pt-4">
          <div className="flex items-center gap-2 border-b border-slate-200 dark:border-[#1e2638] pb-3">
            ...
            <h2 className="text-base font-bold text-slate-900 dark:text-white uppercase tracking-wider text-xs">
              For Creators (Campaign Bundles)
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
            {creatorPlans.map((plan, idx) => renderCard(plan, idx))}
          </div>
        </div>

        {/* FAQ Accordion Section */}
        <div className="max-w-3xl mx-auto border-t border-slate-200 dark:border-[#1e2638] pt-16">
          <div className="text-center mb-10">
            <div className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-white dark:bg-[#131722] border border-slate-200 dark:border-[#1e2638] text-slate-600 dark:text-slate-400 mb-3 shadow-sm">
              <HelpCircle className="w-5 h-5" />
            </div>
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Frequently Asked Questions</h2>
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Have concerns regarding billing pipelines? Find instant clarity below.</p>
          </div>
          ...
          <div className="space-y-3">
            {faqs.map((faq, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="bg-white dark:bg-[#131722] border border-slate-200 dark:border-[#1e2638] rounded-2xl overflow-hidden transition-colors duration-200 shadow-sm"
                >
                  <button
                    onClick={() => toggleFaq(idx)}
                    className="w-full flex items-center justify-between text-left p-4.5 gap-4 text-slate-800 dark:text-slate-200 hover:text-slate-950 dark:hover:text-white transition cursor-pointer"
                  >
                  ...
                  <div
                    className={`transition-all duration-300 ease-in-out overflow-hidden ${
                      isOpen ? 'max-h-40 border-t border-slate-200 dark:border-[#1e2638]' : 'max-h-0'
                    }`}
                  >
                    <div className="p-4 text-xs text-slate-600 dark:text-slate-400 leading-relaxed bg-slate-50 dark:bg-[#0c0f17]">
                      {faq.answer}
                    </div>
                  </div>
                </div>
      ...
      {showStripeModal && selectedPkg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
          <div className="max-w-md w-full p-6 sm:p-7 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#131722] shadow-2xl space-y-5 text-slate-900 dark:text-white">
            ...
            {/* Order Summary */}
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-[#0c0f17] border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
              <div>
            ...
            <form onSubmit={handleStripePay} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-[#0c0f17] border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-[#0c0f17] border border-slate-200 dark:border-slate-800 rounded-xl pl-3.5 pr-10 py-2.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors font-mono"
                  />
                  <Lock className="w-4 h-4 text-slate-400 dark:text-slate-500 absolute right-3 top-3" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="12/28"
                    className="w-full bg-slate-50 dark:bg-[#0c0f17] border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors font-mono text-center"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">

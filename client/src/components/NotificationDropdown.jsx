  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="relative p-2.5 rounded-xl bg-slate-100 border border-slate-200 text-slate-600 hover:text-slate-900 hover:border-indigo-500/50 transition-all focus:outline-none"
        title="Notifications"
      >
        ...
        <div className="absolute right-0 mt-3 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          <div className="p-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
            <div className="flex items-center space-x-2">
              ...
              {unreadCount > 0 && (
                <span className="px-2 py-0.5 text-xs bg-indigo-100 text-indigo-700 font-semibold rounded-full border border-indigo-200">
                  {unreadCount} new
                </span>
              )}
            </div>
            {unreadCount > 0 && (
              <button
                ...
                className="text-xs text-indigo-600 hover:text-indigo-800 flex items-center space-x-1 font-medium transition-colors"
              >
              ...
              notifications.map((item) => (
                <div
                  ...
                  className={`p-4 hover:bg-slate-50 cursor-pointer transition-colors flex items-start space-x-3 ${
                    !item.isRead ? 'bg-indigo-50/60 border-l-2 border-indigo-600' : ''
                  }`}
                >
                  <div className="p-2 rounded-lg bg-indigo-100 text-indigo-600 mt-0.5 flex-shrink-0">
                    <Clock className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-slate-800 leading-relaxed font-normal">{item.message}</p>
                    <div className="flex items-center justify-between mt-2 text-[10px] text-slate-500">
                      <span>{new Date(item.time || item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      <span className="text-indigo-600 flex items-center hover:underline">
